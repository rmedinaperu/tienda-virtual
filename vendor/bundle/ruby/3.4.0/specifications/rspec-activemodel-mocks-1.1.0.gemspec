# -*- encoding: utf-8 -*-
# stub: rspec-activemodel-mocks 1.1.0 ruby lib

Gem::Specification.new do |s|
  s.name = "rspec-activemodel-mocks".freeze
  s.version = "1.1.0".freeze

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["David Chelimsky".freeze, "Andy Lindeman".freeze, "Thomas Holmes".freeze]
  s.cert_chain = ["-----BEGIN CERTIFICATE-----\\nF3MdtaDehhjC\n-----END CERTIFICATE-----\n".freeze]
  s.date = "2018-09-17"
  s.description = "RSpec test doubles for ActiveModel and ActiveRecord".freeze
  s.email = "rspec@googlegroups.com".freeze
  s.homepage = "https://github.com/rspec/rspec-activemodel-mocks".freeze
  s.licenses = ["MIT".freeze]
  s.rdoc_options = ["--charset=UTF-8".freeze]
  s.rubygems_version = "2.7.6".freeze
  s.summary = "rspec-activemodel-mocks-1.1.0".freeze

  s.installed_by_version = "3.6.2".freeze

  s.specification_version = 4

  s.add_runtime_dependency(%q<activesupport>.freeze, [">= 3.0".freeze])
  s.add_runtime_dependency(%q<activemodel>.freeze, [">= 3.0".freeze])
  s.add_runtime_dependency(%q<rspec-mocks>.freeze, [">= 2.99".freeze, "< 4.0".freeze])
  s.add_development_dependency(%q<rake>.freeze, ["~> 10.0.0".freeze])
  s.add_development_dependency(%q<cucumber>.freeze, ["~> 1.3.5".freeze])
  s.add_development_dependency(%q<aruba>.freeze, ["~> 0.4.11".freeze])
  s.add_development_dependency(%q<ZenTest>.freeze, ["~> 4.9.5".freeze])
  s.add_development_dependency(%q<activerecord>.freeze, [">= 3.0".freeze])
end
